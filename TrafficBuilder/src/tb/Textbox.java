...
public class Textbox {
	...
	public void paint(Graphics2D graph2, ExtendedGraphics2D exGraph){
		...
		exGraph.setFont(Main.usingFont.deriveFont(Font.PLAIN, 100f));
		double s1HeightPer1 = ((double) exGraph.getStringBounds(this.text, 0, 0).height) / 100;
		exGraph.setFont(Main.usingFont.deriveFont(Font.PLAIN, (float) ((this.size.height - borderSize * 2) / s1HeightPer1)));
		String textToPaint = "";
		...
		if (this.text == "") {
			...
			while(counter >= 0 && exGraph.getStringBounds(this.text.toCharArray()[counter] + textToPaint, 0, 0).width < this.size.width - borderSize * 2){
			...
			if(this.cursorPosition != 0){
				...
				while(counter >= this.lastViewPosition){
					if(exGraph.getStringBounds(this.text.toCharArray()[counter] + textToPaint, 0, 0).width < this.size.width - borderSize * 2){
						textToPaint = this.text.toCharArray()[counter] + textToPaint;
						counter--;
					}else{break;}
				}
			...
			while(counter < this.lastViewLenght + this.lastViewPosition && this.text.length() > counter){
				if(exGraph.getStringBounds(textToPaint + this.text.toCharArray()[counter], 0, 0).width < this.size.width - borderSize * 2){
			...
			counter = this.cursorPosition;
			while(counter < this.text.length() && exGraph.getStringBounds(textToPaint + this.text.toCharArray()[counter], 0, 0).width < this.size.width - borderSize * 2){
				textToPaint = textToPaint + this.text.toCharArray()[counter];
				counter++;
			}
		...
		graph2.setColor(Color.BLACK);
		final Rectangle finalStringSize = exGraph.getStringBounds(this.text, 0, 0);
		...
		if(this.clicked){
			...
			if(System.currentTimeMillis() % 1000 < 500){
				int cursorX = this.size.width / 80 + this.position.x + borderSize + exGraph.getStringBounds(this.text.substring(viewPosition, this.cursorPosition), 0, 0).width;
